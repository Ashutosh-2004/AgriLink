                ...
                <section>
                  ...
                  <div className="bg-primary/5 border border-primary/15 rounded-xl p-4 space-y-3">
                    ...
                    <div className="bg-card rounded-lg p-3">
                      <p className="text-[9px] text-muted-foreground uppercase mb-2">Government ID / Farmer Verification</p>
                      {user.government_id_url ? (
                        <div className="space-y-2">
                          {/* Preview thumbnail */}
                          {/\.(jpg|jpeg|png|webp|gif)(\?|$)/i.test(user.government_id_url) ? (
                            <a href={user.government_id_url} target="_blank" rel="noopener noreferrer">
                              <img
                                src={user.government_id_url}
                                alt="Government ID"
                                className="w-full max-h-40 object-contain rounded-lg border border-border bg-muted/30 cursor-pointer hover:opacity-90 transition-opacity"
                                onError={e => { (e.target as HTMLImageElement).style.display = "none"; }}
                              />
                            </a>
                          ) : (
                            <div className="w-full py-3 rounded-lg border border-border bg-muted/30 flex items-center justify-center gap-2 text-xs text-muted-foreground">
                        ...
                        <div className="flex items-center gap-2 py-3 px-2 rounded-lg bg-orange-500/5 border border-orange-500/20">
                          <span className="text-lg">⚠️</span>
                          <p className="text-xs text-orange-600 font-semibold">ID not uploaded — cannot verify farmer identity</p>
                        </div>
                      )}
                    </div>
                    {/* Verify toggle */}
